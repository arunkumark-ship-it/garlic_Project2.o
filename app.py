# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  Garlic Order & Delivery Platform  —  app.py                               ║
# ║  Single-file build: NO subfolders, NO local imports, zero ModuleNotFound   ║
# ║  Works on Streamlit Cloud, local, and any Python 3.9+ environment          ║
# ╚══════════════════════════════════════════════════════════════════════════════╝

# ── Standard library only at the top ─────────────────────────────────────────
import os
import uuid
import hashlib
...
:root{
  --green:#1a7f4b; --green2:#25a862; --amber:#854f0b;
  --blue:#185fa5;  --border:#c8e6d4; --bg:#eef5f0;
...
.map-frame{ border-radius:12px; overflow:hidden;
            border:2px solid var(--border); margin-top:.5rem; }
...
DEFAULTS = {
    "logged_in":     False,
    "user":          None,       # dict: uid, name, role, phone
    "driver_id":     None,
    "driver_active": True,
    "active_stop":   0,
    "cust_data":     {},
...
def write_admin_log(admin_uid, action, entity, entity_id, old="", new="", notes=""):
    ...
    append_row("admin_log",
               [log_id, ts, admin_uid, action, entity, str(entity_id),
                str(old), str(new), notes])


# ═══════════════════════════════════════════════════════════════════════════════
#  AUTH  —  hashing, UID generation, login/register
# ═══════════════════════════════════════════════════════════════════════════════
...
def register_user(name, phone, role, password):
    ...
    if role in ("sales executive","delivery Driver"):
        append_row(role_key,[uid, name, phone, role, pw_h, ts])
    return uid, None
...
#  UI HELPERS
# ═══════════════════════════════════════════════════════════════════════════════
def sl(label: str, color: str = "") -> str:
...
def page_admin():
    ...
    with tabs[0]:
        ...
        if not df_sku.empty:
            ...
            avg = df_sku["Price"].apply(
                lambda x: float(str(x).replace("₹","").replace(",","") or 0)).mean()
        ...
        if df_sku.empty:
            ...
            for idx, row in df_sku.iterrows():
                ...
                new_p = c3.number_input("₹", value=float(str(row["Price"]).replace("₹","") or 0),
                                        step=1.0, key=f"skp{idx}", label_visibility="collapsed")
                is_active = str(row.get("Active","")).lower()=="true"
                c4.markdown(pill("Active","pill-on") if is_active else pill("Off","pill-off"),
                            unsafe_allow_html=True)
                if c5.button("Disable" if is_active else "Enable", key=f"skt{idx}"):
                    new_status = "false" if is_active else "true"
                    update_row("skus","SKU Code",row["SKU Code"],{"Active":new_status,"Price":new_p})
                    write_admin_log(user["uid"],
                                    ("Disable" if is_active else "Enable")+" SKU",
        ...
        with st.expander("➕ Create new trip"):
            ...
            if st.button("Create trip", type="primary", key="tr_btn"):
                if not tr_id or not sel_ids:
                    ...
                    append_row("trips",[tr_id, str(tr_date), tr_city,
                                        ",".join(sel_ids),"","","Assigned",
                                        user["uid"],
    ...
    with tabs[2]:
        ...
        if act_df.empty:
            st.warning("🔴 No drivers online right now. Drivers go Active when they log in.")
        else:
        ...
        if trips_df.empty or act_df.empty:
            st.info("Create trips and wait for active drivers before assigning.")
        else:
            ...
            if unassigned.empty:
                ...
                with ac1:
                    sel_trip = st.selectbox("Select trip",
                                            unassigned["Trip ID"].tolist(), key="asgn_trip")
                    if sel_trip:
                ...
                with ac2:
                    drv_opts = act_df.apply(
                        lambda r: f"{r['Full Name']} ({r['Driver ID']})", axis=1).tolist()
                    drv_ids  = act_df["Driver ID"].tolist()
                    sel_drv_lbl = st.selectbox("Select driver", drv_opts, key="asgn_drv")
                    sel_drv_id  = drv_ids[drv_opts.index(sel_drv_lbl)] if sel_drv_lbl else ""
                    drv_name    = act_df[act_df["Driver ID"]==sel_drv_id]["Full Name"].values[0] \
                                  if sel_drv_id else ""
                if st.button("✅ Assign Driver", type="primary", key="asgn_btn"):
                    update_row("trips","Trip ID", sel_trip,
                               {"Driver UID": sel_drv_id,"Driver Name": drv_name,"Status":"Assigned"})
                    write_admin_log(user["uid"],"ASSIGN DRIVER","Trip",
                                    sel_trip,"",sel_drv_id, drv_name)
...
def page_sales():
    ...
    with tabs[0]:
        ...
        with lc1:
            lk_id  = st.text_input("Customer ID", placeholder="CUST-XXXXXX", key="lk_id")
        with lc2:
            lk_mob = st.text_input("OR search by mobile", placeholder="10-digit", key="lk_mob")
        with lc3:
            st.write(""); st.write("")
        ...
        with oc1:
            o_id   = st.text_input("Order ID (auto)", value=gen_order_id(),
                                   disabled=True, key="o_id")
            o_date = st.date_input("Order date", value=date.today(), key="o_date")
        with oc2:
            cities = ["Bengaluru","Mysuru","Hubli","Mangaluru","Hassan","Tumkur"]
            city_default = cust.get("City","Bengaluru")
            city_idx = cities.index(city_default) if city_default in cities else 0
            o_city = st.selectbox("City *", cities, index=city_idx, key="o_city")
        ...
        # Customer fields (auto-filled, read-only)
        ...
        if df_sku.empty:
            ...
            sc1,sc2,sc3 = st.columns(3)
            with sc1:
                sku_opts = df_sku["SKU Code"].tolist()
                sel_sku  = st.selectbox("SKU *", sku_opts, key="o_sku")
            ...
            with sc2:
                ...
                st.text_input("Unit price (admin rate)",
                              value=f"₹{sku_price:.2f}", disabled=True, key="o_price")
            ...
            if st.button("✅ Submit Order", type="primary",
                         use_container_width=True, key="o_submit"):
                if not cust:
    ...
    with tabs[1]:
        ...
        with sc1:
            co_search = st.text_input("Search existing by mobile", key="co_search")
        with sc2:
            st.write(""); st.write("")
        ...
        with nc3:
            co_addr  = st.text_input("Shop address (full) *",
                placeholder="House/street/landmark…", key="co_addr")
        if co_addr:
            st.markdown(map_embed(co_addr, 220), unsafe_allow_html=True)
            st.caption("📍 Verify the pin, then submit.")

        if st.button("✅ Onboard Customer", type="primary",
                     use_container_width=True, key="co_btn"):
...
def page_delivery():
    ...
    with tabs[0]:
        ...
        if st.button(tog_lbl, key="active_tog"):
            ...
            if drv_id:
                set_driver_status(drv_id,"Active" if is_active else "Offline")
            st.rerun()

        if not is_active:
            st.warning("You are offline. Tap the button above to go active and receive trips.")
            return
        ...
        st.info(f"**Trip:** {trip['Trip ID']} · **{trip['City']}** · **{trip['Date']}** · "
                f"{len(shop_ids)} stops")
        ...
        # Count already-done stops
        done_count = sum(
            1 for sid in shop_ids
        ...
        for i, sid in enumerate(shop_ids):
            ...
            is_done   = bool(order and str(order.get("Delivery Status",""))
                             not in ("Pending",""))
            ...
            with st.container():
                ...
                with r1:
                    st.markdown(f"**{icon} Stop {i+1}** — "
                                f"{shop.get('Shop Name','') if shop else sid}")
                    if shop:
                        st.caption(f"📍 {shop.get('Shop Address','')}")
                    if order:
                        st.caption(f"SKU: {order.get('SKU','')} · "
                                   f"Qty: {order.get('OrderedQty','')} · "
                                   f"₹{order.get('OrderTotal','')}")
                with r2:
                    st.markdown(pill(stat, p_cls), unsafe_allow_html=True)

            if is_cur and not is_locked:
                ...
                with st.form(key=f"del_form_{i}"):
                    ...
                    with df1:
                        d_reach = st.time_input("Reach time *",
                                                value=datetime.now().time())
                        d_ddate = st.date_input("Delivered date", value=date.today())
                    with df2:
                        d_status = st.selectbox("Status *",
    ...
    with tabs[1]:
        ...
        with ds1:
            do_search_val = st.text_input("Search existing driver by mobile", key="do_search")
        with ds2:
            st.write(""); st.write("")
            do_do_search = st.button("🔍 Search", key="do_search_btn")
        if do_do_search and do_search_val:
            ex = find_row("driver_onboard","Mobile", do_search_val.strip())
            if ex:
                ...
                st.json({"Name":ex.get("Full Name"),"Vehicle":ex.get("Vehicle Type"),
                         ...
                         "Active":ex.get("Active Status")})
            else:
                st.info("Not found — fill form below.")

        st.divider()
        st.markdown("#### New driver")
        ...
        with dn2:
            do_veh   = st.selectbox("Vehicle type",
                ["Bike","Auto","Van","Truck","Mini-Truck"], key="do_veh")
        ...
        if st.button("✅ Onboard Driver", type="primary",
                     use_container_width=True, key="do_btn"):
